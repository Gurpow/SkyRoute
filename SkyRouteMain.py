from graph_search_functions import bfs, dfs
from vc_metro_dictionary import vc_metro
from vc_landmark_dictionary import vc_landmarks
from landmark_choices_dictionary import landmark_choices
...
def new_route(start_point=None, end_point=None):
    ...
    if shortest_route is not None:
        ...
        print("Unfortunately, there is currently no path between {0} and {1} due to maintenance.".format(start_point,
                                                                                                         end_point))
    again = input("Would you like to see another route? Enter 'y' for yes / 'n' for no: ")
    if again == 'y':
        show_landmarks()
...
def show_landmarks():
    see_landmarks = input("Would you like to see the list of landmarks again? Enter 'y' for yes / 'n' for no: ")
    if see_landmarks == 'y':
